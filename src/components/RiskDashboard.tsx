...
const RiskDashboard = () => {
  const overallRisk = 23;

  return (
    <section className="py-24 px-6 bg-secondary/20">
      <div className="container mx-auto max-w-6xl">
        <div className="text-center mb-12">
          <span className="text-sm font-mono text-primary mb-2 block">RISK ASSESSMENT</span>
          ...
          <div className="glass-card glow-border p-8 flex flex-col items-center justify-center">
            <div className="relative w-36 h-36 mb-4">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl font-bold text-foreground">{overallRisk}%</span>
              ...
              {riskFactors.map((f) => (
                <div key={f.label}>
                  ...
                  <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${f.color} transition-all`} style={{ width: `${f.score}%` }} />
                  </div>
                </div>
              ))}
            </div>
              ...
              {deployments.map((d) => (
                <div key={d.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/40">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${d.status === "success" ? "bg-success" : "bg-warning"}`} />
                    <div>
                      <span className="text-sm font-medium text-foreground">{d.service}</span>
                      <span className="text-xs text-muted-foreground ml-2 font-mono">{d.id}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className={`text-xs font-mono ${d.risk > 50 ? "text-warning" : "text-success"}`}>{d.risk}%</span>
